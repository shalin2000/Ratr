...
export default class InProgressScreen extends React.Component {
  ...
  fetchDataFromApi = () => {
    const url = "https://ratr-app21.herokuapp.com/api/list/";
  ...
  updateToAPI(item){
    ...
    fetch("https://ratr-app21.herokuapp.com/api/list/"+item.id+'/', {
      method: 'PUT', // or 'PUT'
      headers: {
  ...
  setUpdateModalVisible = (visible,item,txt) => {
    ...
    if (txt === 'close'){
      this.setState({ selectedDone: false, selectedBookmark: false, selectedReading: true })
    }
    if (txt === 'open'){
    ...
    this.setState({ updateModalVisible: visible, selectedItem: item, userComment: item.user_comment,
                    selectedDone: false, selectedBookmark: false, selectedReading: true });
  }
  ...
  render() {
    ...
    var userData = this.state.user !== null ? this.state.myListData.filter(x => (x.email === this.state.user.email) && (x.user_progress === 'Reading')) : null;
